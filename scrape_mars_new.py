...
#create one master function to collect individual elements as Homework instruction is not enough
def scrape():
    web_output = {}
    newsoutput = marsNews()
    web_output["news_title"] = newsoutput[0]
    web_output["news_para"] = newsoutput[1]
    web_output["mars_image"] = featuredImage()
    web_output["mars_weather"] = marsWeather()
    web_output["mars_facts"] = marsFacts()
    web_output["mars_hemisphere"] = marsHemisphere()
    return web_output
...
def marsNews():
    ...
    newsoutput = [news_title, news_paragraph]
    return newsoutput
